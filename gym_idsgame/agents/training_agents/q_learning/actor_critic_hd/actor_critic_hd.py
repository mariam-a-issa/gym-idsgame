...
from gym_idsgame.agents.training_agents.q_learning.abstract_actor_critic_hd_agent import AbstractActorCriticHDAgent
from QHDModel import QHDModel

class ActorCriticHDAgent(AbstractActorCriticHDAgent):
    """
    ...
    """
    def __init__(self, env: IdsGameEnv, config: AbstractActorCriticHDAgentConfig, hp_to_tune):
        """
        Initialize environment and hyperparameters

        :param config: the configuration
        """
        super(ActorCriticHDAgent, self).__init__(env, config)
        ...
        # Set up Buffer
        self.buffer = ReplayBuffer(config.actor_critic_hd_config.replay_memory_size)

        # Metrics Tracking
        self.tensorboard_writer = SummaryWriter(self.config.actor_critic_hd_config.tensorboard_dir)
    ...
    def initialize_models(self) -> None:
        ...
        # print(f"***input_dim: {self.config.actor_critic_hd_config.input_dim}, attacker_output_dim: {self.config.actor_critic_hd_config.attacker_output_dim}, defender_output_dim: {self.config.actor_critic_hd_config.defender_output_dim}")

        # Initialize models
        self.attacker_q_network = QHDModel(
            self.config.actor_critic_hd_config.input_dim, # Number of features: (# of nodes) * (# of attacks + 1)
            self.config.actor_critic_hd_config.attacker_output_dim, # 33
            lr=self.config.lr,
            lr_decay_rate=self.config.actor_critic_hd_config.lr_decay_rate,
            dimension=self.config.actor_critic_hd_config.dimension)
        self.defender_q_network = QHDModel(
            self.config.actor_critic_hd_config.input_dim, # 88
            self.config.actor_critic_hd_config.defender_output_dim, # 88
            lr = self.config.lr,
            lr_decay_rate=self.config.actor_critic_hd_config.lr_decay_rate,
            dimension=self.config.actor_critic_hd_config.dimension)

        # Delay Models, might delete this
        # self.attacker_target_network = QHDModel(
        #     self.config.dqn_config.input_dim,
        #     self.config.dqn_config.attacker_output_dim,
        #     dimension=self.dimension)
        # self.defender_target_network = QHDModel(
        #     self.config.dqn_config.input_dim,
        #     dimension=self.dimension)

        # Specify device
        if torch.cuda.is_available() and self.config.actor_critic_hd_config.gpu:
            device = torch.device("cuda:0")
    ...
    #                   attacker: bool = True) -> torch.Tensor:
    #     """
    ...
    #         # self.attacker_target_network.eval()
    #         reward = torch.tensor(r_attacker_batch).float()
    #         state = torch.tensor(s_attacker_batch).float()
    #         next_state = torch.tensor(s2_attacker_batch).float()
    #     else:
    #         # self.defender_q_network.train()
    #         # self.defender_q_network.eval()
    #         reward = torch.tensor(r_defender_batch).float()
    #         state = torch.tensor(s_defender_batch).float()
    #         next_state = torch.tensor(s2_defender_batch).float()
    #
    #     # Move to GPU if using GPU
    #     if torch.cuda.is_available() and self.config.qhd_config.gpu:
    #         device = torch.device("cuda:0")
    ...
    #     if attacker:
    #         target = self.attacker_q_network(state) # get the scores for the given state
    #     else:
    #         target = self.defender_q_network(state)
    #
    ...
    #
    #     for i in range(self.config.qhd_config.batch_size):
    ...
    def get_action(self, state: np.ndarray, eval: bool = False, attacker: bool = True) -> int:
        ...
        # print(f"***num_attack_actions: {self.env.num_attack_actions}, num_defense_actions: {self.env.num_defense_actions}")
        if attacker:
        ...
        if (np.random.rand() < self.config.epsilon and not eval) \
                or (eval and np.random.random() < self.config.eval_epsilon): # TODO: review this logic, not sure if necessary
            return np.random.choice(legal_actions)

        # with torch.no_grad():
        if attacker:
            act_values = self.attacker_q_network.get_q_value(state)
        else:
            act_values = self.defender_q_network.get_q_value(state)

        # print(f"**act_values: {len(act_values)} {act_values}")
        # print(f"***legal_actions: {legal_actions}")
        # print(f"***act_values[legal_actions]: {len(act_values[legal_actions])} {act_values[legal_actions]}")
        # print(f"***torch.argmax(act_values[legal_actions]): {torch.argmax(act_values[legal_actions])}")

        return legal_actions[torch.argmax(act_values[legal_actions]).item()]

    def train(self) -> ExperimentResult:
        """
        Runs the QHD algorithm
        ...
        # Tracking metrics across all episodes
        ...
        for episode in range(self.config.num_episodes):
            ...
            while not done:
                ...
                action = (attacker_action, defender_action) # TODO: I am confusion, why is the opponent always choosing 0???

                # TODO *** Take a step in the environment and update to next state ***
                next_obs, reward, done, _ = self.env.step(action) # TODO *** that action is a tuple ***
                next_obs_attacker, next_obs_defender = next_obs
                updated_state_attacker = self.update_state(next_obs_attacker, next_obs_defender, attacker=True, state=[])
                updated_state_defender = self.update_state(next_obs_attacker, next_obs_defender, attacker=False, state=[])
                updated_states = (updated_state_attacker, updated_state_defender)

                # Add transition to replay memory
                self.buffer.add_tuple(obs, action, reward, done, updated_states)

                # Sample random mini_batch of transitions from replay memory
                # minibatch = self.buffer.sample(self.config.dqn_config.batch_size)

                # Update Model
                if self.buffer.count >= self.config.actor_critic_hd_config.batch_size:
                    if self.config.attacker:
                        # loss = self.training_step(minibatch, attacker=True) =
                        # episode_attacker_loss += loss.item()
                        self.attacker_q_network.update_model(self.buffer, self.config.actor_critic_hd_config.batch_size, attacker=True)

                    if self.config.defender:
                        # loss = self.training_step(minibatch, attacker=False)
                        # episode_defender_loss += loss.item()
                        self.defender_q_network.update_model(self.buffer, self.config.actor_critic_hd_config.batch_size, attacker=False)
                ...
                obs = updated_states
                attacker_obs, defender_obs = updated_states
            ...
            # TODO *** End of Episode Updates/Logistics ***

            # Update Learning Rate (if applicable)
            if self.config.actor_critic_hd_config.lr_exp_decay:
                self.attacker_q_network.lr *= self.attacker_q_network.lr_decay_rate
                # TODO: in original code: why do they only update the attacker's learning rate
                self.defender_q_network.lr *= self.defender_q_network.lr_decay_rate
            ...
            if episode % self.config.train_log_frequency == 0:
                ...
                #TODO: come back and add named_parameters to QHD model
                # if self.config.attacker:
                ...
                #                                               episode)named_parameters

                episode_attacker_rewards = []
                episode_defender_rewards = []
                episode_steps = []
                self.num_train_games = 0 # This is reset at each logging frequency (vs. num_train_games_total is reset to 0)
                self.num_train_hacks = 0 # This is reset at each logging frequency

            # Update Delay Model
            if episode % self.config.actor_critic_hd_config.target_network_update_freq == 0:
                self.config.logger.info("Updating target network")
                if self.config.attacker:
                    self.attacker_q_network.update_delay_model()
                else:
                    self.defender_q_network.update_delay_model()

            # Run evaluation every <self.config.eval_frequency> episodes
            # TODO: I'm going to skip eval, since it does the same thing as line 503-534, but generates gif/videos
            ...
            if episode % self.config.checkpoint_freq == 0:
                ...
                #         self.config.save_dir + "/qhd_train_checkpoint_" + checkpoint_counter + ".csv")
                #     self.eval_result.to_csv(
                #         self.config.save_dir + "/qhd_eval_checkpoint_" + checkpoint_counter + ".csv")

            # Reset environment for the next episode and update game stats
            done = False
            obs = self.env.reset(update_stats=True)
            attacker_obs, defender_obs = obs
            updated_state_attacker = self.update_state(attacker_obs, defender_obs, attacker=True, state=[])
            updated_state_defender = self.update_state(attacker_obs, defender_obs, attacker=False, state=[])
            obs = (updated_state_attacker, updated_state_defender)
            attacker_obs, defender_obs = obs
            self.outer_train.update(1) # has to do with tqdm

            # epsilon Decay
            if self.config.epsilon > self.config.min_epsilon:
                self.config.epsilon = self.config.epsilon * self.config.epsilon_decay

        # TODO NOTE: ignore below
        self.config.logger.info("Training Complete")

        # Final evaluation (for saving Gifs etc)
        # self.eval(self.config.num_episodes-1, log=False) # TODO: might need to look into
        ...
        if self.config.save_dir is not None:
            ...
            if self.hp_to_tune == "epsilon_decay_":
                x = str(self.config.epsilon_decay)
            elif self.hp_to_tune == "target_freq_":
                x = str(self.config.actor_critic_hd_config.target_network_update_freq)
            elif self.hp_to_tune == "lr_":
                x = str(self.config.lr)
            else:
                x = str(time.time())
            self.train_result.to_csv(self.config.save_dir + "/actor_critic_hd_train_results_" + self.hp_to_tune + x + ".csv")
            self.eval_result.to_csv(self.config.save_dir + "/actor_critic_hd_eval_results_" + self.hp_to_tune + x + ".csv")
    ...
    # TODO NOTE: did not go over because I don't think I need to
    def update_state(self,
                     attacker_obs: np.ndarray = None,
                     defender_obs: np.ndarray = None,
                     state: np.ndarray = None,
                     attacker: bool = True) -> np.ndarray:
        """
        ...
        # Zero mean
        if self.config.actor_critic_hd_config.zero_mean_features:
        ...
        # Normalize
        if self.config.actor_critic_hd_config.normalize_features:
            if not self.env.local_view_features() or not attacker:
        ...
        if self.env.fully_observed() or \
            ...
            if self.config.actor_critic_hd_config.merged_ad_features:
                if not self.env.local_view_features() or not attacker:
                ...
                if self.env.idsgame_config.reconnaissance_bool_features:
                    ...
                    features = f
                if self.config.actor_critic_hd_config.state_length == 1:
                    return features
                if len(state) == 0:
                    s = np.array([features] * self.config.actor_critic_hd_config.state_length)
                    return s
                ...
                if self.config.actor_critic_hd_config.state_length == 1:
                    return f
                if len(state) == 0:
                    s = np.array([f] * self.config.actor_critic_hd_config.state_length)
                    return s
                # if not self.env.local_view_features() or not attacker:
            ...
            if len(state) == 0:
                if attacker:
                    return np.array([attacker_obs] * self.config.actor_critic_hd_config.state_length)
                else:
                    return np.array([defender_obs] * self.config.actor_critic_hd_config.state_length)
            if attacker:
