...
class AbstractQHDAgentConfig:
    ...
    def __init__(self, gamma :float = 0.8, lr:float = 0.1, epsilon :float =0.9, render :bool =False,
        ...
        :param qhd_config: configuration for QHD
    ...
    def to_csv(self, file_path: str) -> None:
        ...
        with open(file_path, "w") as f:
            ...
            if self.qhd_config is not None:
                writer.writerow(["input_dim", str(self.qhd_config.input_dim)])
                writer.writerow(["output_dim", str(self.qhd_config.attacker_output_dim)])
                writer.writerow(["replay_memory_size", str(self.qhd_config.replay_memory_size)])
                writer.writerow(["batch_size", str(self.qhd_config.batch_size)])
                writer.writerow(["target_network_update_freq", str(self.qhd_config.target_network_update_freq)])
                writer.writerow(["gpu", str(self.qhd_config.gpu)])
                writer.writerow(["tensorboard", str(self.qhd_config.tensorboard)])
                writer.writerow(["tensorboard_dir", str(self.qhd_config.tensorboard_dir)])
                writer.writerow(["lr_exp_decay", str(self.qhd_config.lr_exp_decay)])
                writer.writerow(["lr_decay_rate", str(self.qhd_config.lr_decay_rate)])
    ...
    def hparams_dict(self):
        ...
        if self.qhd_config is not None:
            hparams["input_dim"] = self.qhd_config.input_dim
            hparams["output_dim"] = self.qhd_config.attacker_output_dim
            hparams["replay_memory_size"] = self.qhd_config.replay_memory_size
            hparams["batch_size"] = self.qhd_config.batch_size
            hparams["target_network_update_freq"] = self.qhd_config.target_network_update_freq
            hparams["gpu"] = self.qhd_config.gpu
            hparams["lr_exp_decay"] = self.qhd_config.lr_exp_decay
            hparams["lr_decay_rate"] = self.qhd_config.lr_decay_rate
        return hparams
