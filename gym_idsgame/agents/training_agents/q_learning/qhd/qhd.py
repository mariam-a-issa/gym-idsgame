...
from gym_idsgame.agents.training_agents.q_learning.abstract_qhd_agent import AbstractQHDAgent
from QHDModel import QHDModel

class QHDAgent(AbstractQHDAgent):
    """
    ...
    """
    def __init__(self, env: IdsGameEnv, config: AbstractQHDAgentConfig, hp_to_tune):
        """
        Initialize environment and hyperparameters

        :param config: the configuration
        """
        super(QHDAgent, self).__init__(env, config)
        ...
        # Set up Buffer
        self.buffer = ReplayBuffer(config.qhd_config.replay_memory_size)

        # Metrics Tracking
        self.tensorboard_writer = SummaryWriter(self.config.qhd_config.tensorboard_dir)
    ...
    def initialize_models(self) -> None:
        ...
        # print(f"***input_dim: {self.config.qhd_config.input_dim}, attacker_output_dim: {self.config.qhd_config.attacker_output_dim}, defender_output_dim: {self.config.qhd_config.defender_output_dim}")

        # Initialize models
        self.attacker_q_network = QHDModel(
            self.config.qhd_config.input_dim, # Number of features: (# of nodes) * (# of attacks + 1)
            self.config.qhd_config.attacker_output_dim, # 33
            lr=self.config.lr,
            lr_decay_rate=self.config.qhd_config.lr_decay_rate,
            dimension=self.config.qhd_config.dimension)
        self.defender_q_network = QHDModel(
            self.config.qhd_config.input_dim, # 88
            self.config.qhd_config.defender_output_dim, # 88
            lr = self.config.lr,
            lr_decay_rate=self.config.qhd_config.lr_decay_rate,
            dimension=self.config.qhd_config.dimension)
    ...
    def train(self) -> ExperimentResult:
        ...
        for episode in range(self.config.num_episodes):
            ...
            while not done:
                ...
                # Update Model
                if self.buffer.count >= self.config.qhd_config.batch_size:
                    if self.config.attacker:
                        # loss = self.training_step(minibatch, attacker=True) =
                        # episode_attacker_loss += loss.item()
                        self.attacker_q_network.update_model(self.buffer, self.config.qhd_config.batch_size, attacker=True)

                    if self.config.defender:
                        # loss = self.training_step(minibatch, attacker=False)
                        # episode_defender_loss += loss.item()
                        self.defender_q_network.update_model(self.buffer, self.config.qhd_config.batch_size, attacker=False)
            ...
            if self.config.qhd_config.lr_exp_decay:
            ...
            # Update Delay Model
            if episode % self.config.qhd_config.target_network_update_freq == 0:
        ...
        if self.config.save_dir is not None:
            ...
            if self.hp_to_tune == "epsilon_decay_":
                ...
                x = str(self.config.qhd_config.target_network_update_freq)
            elif self.hp_to_tune == "lr_":
                x = str(self.config.lr)
            else:
                x = str(time.time())
            self.train_result.to_csv(self.config.save_dir + "/qhd_train_results_" + self.hp_to_tune + x + ".csv")
            self.eval_result.to_csv(self.config.save_dir + "/qhd_eval_results_" + self.hp_to_tune + x + ".csv")
    ...
    def update_state(self,
        ...
        # Zero mean
        if self.config.qhd_config.zero_mean_features:
        ...
        # Normalize
        if self.config.qhd_config.normalize_features:
            if not self.env.local_view_features() or not attacker:
        ...
        if self.env.fully_observed() or \
            ...
            if self.config.qhd_config.merged_ad_features:
                if not self.env.local_view_features() or not attacker:
                ...
                if self.env.idsgame_config.reconnaissance_bool_features:
                    ...
                    features = f
                if self.config.qhd_config.state_length == 1:
                    return features
                if len(state) == 0:
                    s = np.array([features] * self.config.qhd_config.state_length)
                    return s
                ...
                if self.config.qhd_config.state_length == 1:
                    return f
                if len(state) == 0:
                    s = np.array([f] * self.config.qhd_config.state_length)
                    return s
                # if not self.env.local_view_features() or not attacker:
            ...
            if len(state) == 0:
                if attacker:
                    return np.array([attacker_obs] * self.config.qhd_config.state_length)
                else:
                    return np.array([defender_obs] * self.config.qhd_config.state_length)
            if attacker:
